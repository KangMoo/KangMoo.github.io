...
Docker는 개발, 배포, 실행을 모두 일관성 있게 관리해주는 컨테이너 기반의 오픈소스 플랫폼이다.

컨테이너란 간단히 말해서 독립적으로 실행될 수 있는 가장 작은 단위의 소프트웨어다.

Docker는 이 컨테이너를 사용하여 코드를 묶고, 모`든 종속성을 함께 포장하여 애플리케이션을 실행하는 환경을 공급한다.

이로 인해 애플리케이션은 호스트 시스템으로부터 독립적이며, 어떠한 환경에서도 동일하게 작동한다.
